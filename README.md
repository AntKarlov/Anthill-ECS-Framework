...
При использовании данного подхода не рекомендуется реализовывать обмен данными через события. Если вам необходимо реализовать обмен данными между игровыми объектами, то вам следует использовать подход семафоров и передавать данные через переменные или структуры внутри компонентов.


**Node**

Узел — используется чтобы распределять сущности по системам, то есть это фильтр который позволяет передавать сущности в системы если они соотвествуют правилам наличия или отсутствия у них необходимого набора компонентов.
...
Так же мы можем указать каких компонентов не должно быть при этом у сущности, чтобы она попала в определнный узел. Например, изменив узел следующим образом мы можем исключить попадания сущности в любые системы обрабатывающие здоровье если у нее будет компонент бессмертия:
...
Аттрибут `Exclude` позволяет задать один или несколько (максимум 6) компонентов, которых при этом не должно быть у сущности чтобы она соответствовала правилам попадания в данный узел. Таким образом, добавив на лету любой сущности компонент `Immortal` мы можем сделать её бессмертной не удаляя при этом компонент `Health`.


**System**

Системы содержат логику и являются основными процессорами для обработки игровых сущностей и их компонентов. Системы не имеют прямого отношения к движку Unity и их функционал определяется через интерфейсы. Рассмотрим пару из них: `ISystem` и `IExecuteSystem`.
...
сценарии — это способ держать вещи в порядке.
...
После добавления сущности, она будет автоматическ распределена по спискам узлов согласно наличию компонентов. Обновятся даже те списки узлов, которые вы уже получили, так как при извлечении списка нод через `AntEngine.GetNodes<T>()` — вы получаете лишь указатель на список узлов, а не его копию.
...
В обоих приведенных случаях, логика объекта не изменится. А при удалении компонента через стандартный gameObject — вовсе может привести к ошибкам, если сущность ранее бяла добавлена в движок.
...
Например, если ваш персонаж получил урон и вам нужно как-то передать информацию из системы обработки пуль, о точке попадения пули, её скорости и другой технической информации, то вам следует сделать и добавить компонент `HitBulletInfo`, записав в него все данные, чтобы передать их, например в систему создания эффектов:
